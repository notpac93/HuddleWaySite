...
| `Financials.svelte` | 29 | Owner-scoped views load core projections and direct invoices through authenticated APIs; organization/team scope, loading/empty/permission/error/truncation states, debounced search, date/status/currency filters, deterministic sorting, stable-ID selection, responsive pagination, column visibility, tenant-local saved views, audited invoice export, refresh, invoice creation, and row details are wired. Reactive projection/filter dependencies recompute after asynchronous data, filters, and team changes; refresh retains the current projection; tenant/role/scope changes close stale drawers and invalidate pending exports; unchanged exact selections reuse an operation key while changed selections rotate it. The backend now binds that key to a normalized tenant/actor request, replays one persisted checksum-verified payload, rejects changed reuse, and records one deterministic audit. Unsupported exports remain disabled with the missing-contract reason. |
| `FinancialPeriodManager.svelte` | 14 | Owner-only preview, close, and reopen controls require impact confirmation, an internal reason, stable idempotency, correlated state, and refresh of the bounded period projection. |
...
| `TransactionDetails.svelte` | 36 | Direct-invoice creation, line items, server-parity totals, ledger refresh, issue, reminder, offline payment, invoice/core refunds, void, hosted links, and read-only deposit/dispute details use authenticated backend operations. The drawer now resolves real `direct_invoice:<id>` table rows through `recordId`, recomputes draft totals on every line/discount/tax edit, locks all mutable fields in flight, ignores stale tenant/row results, and binds retry keys to normalized payloads. Every mutation requires owner access, an internal audit reason, impact preview and confirmation, support-safe correlated result state, and authoritative refresh; unsupported lifecycle actions remain disabled with an exact reason. |
