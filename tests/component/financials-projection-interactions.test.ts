...
import { activeTenantRole, tenantIdStore } from "../../src/lib/authStore";
...
function invoicePage(invoices: DirectInvoiceRecord[], tenantId = "tenant-a") {
  return {
    tenantId,
    invoices,
    hasMore: false,
...
describe("Financials projection and controls", () => {
  ...
  it("does not request financial records for a viewer and loads when owner authorization arrives", async () => {
    ...
    expect(
      new URL(window.location.href).searchParams.has("financeInvoice"),
    ).toBe(false);
    ...
    expect(backendMocks.directInvoicePage).toHaveBeenCalledWith("tenant-a", {
      limit: 200,
      cursor: undefined,
    });
  });
  ...
  it("retains the current projection during refresh and ignores the old tenant result", async () => {
    ...
    backendMocks.directInvoicePage
      ...
      .mockResolvedValue(invoicePage([invoice("invoice-b")], "tenant-b"));
  ...
  it("uses support-safe load errors and recovers through the retry control", async () => {
    ...
    expect(
      screen.getByText(/Support request: financial-request/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("exports exact selected ids with stable retries, rotates changed scope, and suppresses stale downloads", async () => {
    ...
    expect(
      screen.getByText(/Support request: export-request/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText("Private export worker detail"),
    ).not.toBeInTheDocument();
    const firstKey = backendMocks.createCrmExport.mock.calls[0][1];
    expect(backendMocks.createCrmExport.mock.calls[0][0].selection).toEqual({
      scope: "explicit",
      ids: ["invoice-1"],
    });
    ...
    expect(backendMocks.createCrmExport.mock.calls[2][0].selection).toEqual({
      scope: "explicit",
  ...
  it("wires pagination, page selection, columns, search, status, and URL-backed sorting", async () => {
    ...
    expect(await screen.findByText(/^30 records$/i)).toBeInTheDocument();
    ...
    expect(await screen.findByText(/^15 records$/i)).toBeInTheDocument();
    expect(screen.getByText(/Page 1 of 1 · 0 selected/i)).toBeInTheDocument();
    expect(
      new URL(window.location.href).searchParams.get("financeStatus"),
    ).toBe("paid");

    await fireEvent.input(screen.getByLabelText("Search loaded records"), {
      target: { value: "invoice-02" },
    });
    expect(await screen.findByText(/^1 record$/i)).toBeInTheDocument();
  ...
  it("saves, applies, and deletes tenant-local views without crashing when browser storage rejects a write", async () => {
    ...
    expect(screen.getByText(/View saved in this browser/i)).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/Deleted local view “Open invoices”/i),
    ).toBeInTheDocument();
