...
<div class="fixed inset-0 z-50 flex justify-end bg-slate-950/40" role="presentation" on:click|self={onClose}>
  <div class="h-full w-full max-w-xl overflow-y-auto bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="participant-title">
    ...
    <div class="space-y-5 p-5">
      ...
      {:else}
        {#if errorMessage}<div class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800" role="alert">{errorMessage}</div>{/if}
        ...
        <section class="rounded-xl border border-gray-200 p-4">
          ...
          {#if relationships}
            <div class="mt-4 border-t border-gray-100 pt-4">
              ...
              {#if hasDraft}<p class="mt-2 text-sm text-[var(--crm-brand-link)]">Staged: {selectedTeamName}. Nothing changes until Publish.</p>{/if}
        ...
        <section class="rounded-xl border border-gray-200 p-4">
          ...
          {#if editingAgreement}
            <div class="mt-4 rounded-lg border border-[var(--crm-brand-border)] bg-[var(--crm-brand-surface)] p-4 text-[var(--crm-on-brand-surface)]">
              ...
              <p class="mt-2 text-xs">Paid or processing payments never change. An increase or earlier collection requires the customer’s approval before becoming effective.</p>
