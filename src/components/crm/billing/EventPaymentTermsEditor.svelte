...
<section class="rounded-xl border border-gray-200 bg-white p-4">
  ...
  {#if priceCents > 0}
    ...
    {#if choice === 'installments'}
      ...
      {#if splitAmounts.length}<div class="mt-3 rounded-lg bg-[var(--crm-brand-surface)] p-3 text-sm text-[var(--crm-on-brand-surface)]"><strong>{splitAmounts.length} payments</strong><ol class="mt-2 grid gap-1 sm:grid-cols-2">{#each splitAmounts as amount, index}<li>Payment {index + 1}: {money(amount)}</li>{/each}</ol></div>{/if}
    {/if}
